...
//    auto cboard = VirtualCBoard();
    ...
    auto armorIdentifier = ArmorIdentifier_V3<NumberIdentifier_V1>("/home/alliance/lio_ws/src/ugas/models/NumberIdentifyModelV4.pb");
...
//                auto [rect_x, rect_y] = ArmorPnPSolver::ReProjection(target->Predict(0));
                if (cboard.get_shooter_index() == 0) {
                    yaw += 3.0 / 180.0 * parameters::Pi;
                    pitch += 2.0 / 180.0 * parameters::Pi;
                }
                else {
                    yaw += 1.0 / 180.0 * parameters::Pi;
                    pitch += 2.0 / 180.0 * parameters::Pi;
                }
                std::cout << (int)cboard.get_shooter_index() << '\n';
